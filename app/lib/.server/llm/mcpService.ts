...
export class MCPService {
  ...
  private async _createMCPClient(serverName: string, serverConfig: MCPServerConfig): Promise<MCPClient> {
    ...
    if (validatedConfig.type === 'stdio') {
      return await this._createStdioClient(serverName, validatedConfig);
    } else if (validatedConfig.type === 'sse') {
      return await this._createSSEClient(serverName, validatedConfig);
    } else {
      return await this._createStreamableHTTPClient(serverName, validatedConfig);
    }
  }
  ...
  private async _createClients() {
    ...
    const createClientPromises = Object.entries(this._config?.mcpServers || []).map(
