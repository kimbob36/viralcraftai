...
export async function action({ request, context }: ActionFunctionArgs) {
  ...
  try {
    ...
    const githubToken =
      (process.env as any)?.GITHUB_BUG_REPORT_TOKEN || process.env.GITHUB_BUG_REPORT_TOKEN;
    const targetRepo =
      (process.env as any)?.BUG_REPORT_REPO || process.env.BUG_REPORT_REPO || 'stackblitz-labs/bolt.diy';
